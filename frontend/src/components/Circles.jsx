...
const CircleListItem = ({ circle, isSelected, onSelect }) => (
    <motion.div
        ...
        className={`flex items-center p-3 space-x-3 rounded-lg cursor-pointer transition-colors ${isSelected ? "bg-rose-200/60" : "hover:bg-rose-100/50"}`}
        whileTap={{ scale: 0.98 }}
    >
        <Avatar className="h-11 w-11 border-2 border-white">
            ...
            <AvatarFallback className="bg-rose-200 text-rose-700">{circle.circleName ? circle.circleName[0] : 'C'}</AvatarFallback>
        </Avatar>
        <div className="flex-1 min-w-0">
            <p className="text-md font-semibold text-rose-900 truncate">{circle.circleName}</p>
            <p className="text-sm text-rose-600 truncate">{circle.memberId.length} members</p>
        </div>
    </motion.div>
);
...
const CircleMemoryCard = ({ memory, onCardClick }) => (
    ...
    >
        <img src={memory.mediaUrl} alt={memory.title} className="w-full h-40 object-cover" />
        <div className="p-3">
            <h4 className="font-semibold font-serif text-rose-800 truncate">{memory.title}</h4>
            <p className="text-xs text-rose-600">by {memory.author.fullName}</p>
        </div>
    </motion.div>
);

const CircleDetail = ({ circle, memories, loading, onShareClick, onMemoryClick }) => (
    <motion.div
        key={circle._id}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
    ...
    >
        ...
        <header className="p-4 border-b border-rose-200/80 flex justify-between items-center flex-shrink-0">
            <div className="flex items-center gap-4">
                <Avatar className="h-12 w-12 border-2 border-white">
                    <AvatarImage src={circle.avatarUrl} alt={circle.circleName} />
                    <AvatarFallback className="bg-rose-200 text-rose-700">{circle.circleName[0]}</AvatarFallback>
                </Avatar>
                <div>
                    <h2 className="text-xl font-bold font-serif text-rose-800">{circle.circleName}</h2>
                    <p className="text-sm text-rose-600">{circle.memberId.length} members</p>
                </div>
            </div>
            <Button variant="ghost" size="icon"><Settings className="h-5 w-5 text-rose-600" /></Button>
        </header>

        {/* Memories Content Area */}
        <div className="flex-1 p-4 sm:p-6 overflow-y-auto">
            {loading ? (
                <div className="flex justify-center items-center h-full"><Loader2 className="h-8 w-8 text-rose-500 animate-spin" /></div>
            ) : memories.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {memories.map(memory => (
                        <CircleMemoryCard key={memory._id} memory={memory} onCardClick={onMemoryClick} />
                    ))}
                </div>
            ) : (
                <div className="h-full text-center flex flex-col justify-center items-center">
                    ...
                    <h3 className="text-xl font-semibold text-rose-700">No Memories Yet</h3>
                    <p className="text-rose-500 max-w-xs mt-2">Be the first to share a story, photo, or memory in the "{circle.circleName}" circle.</p>
                </div>
            )}
        </div>
        ...
        <footer className="p-4 border-t border-rose-200/80">
            <Button onClick={onShareClick} className="w-full bg-green-600 hover:bg-green-700">
...
export default function Circles() {
    ...
    const { items: circles, loading: circlesLoading } = useSelector((state) => state.circle);
    const { familyData } = useSelector((state) => state.family);
    const { items: memories, loading: memoriesLoading } = useSelector((state) => state.memories);
    ...
    useEffect(() => {
        if (!selectedCircle && circles && circles.length > 0) {
    ...
    useEffect(() => {
        if (selectedCircle) {
            // We pass an empty search term and the circleId filter
            dispatch(fetchMemories({ searchTerm: "", filters: { circleId: selectedCircle._id } }));
        }
        ...
        <div className="h-screen w-full flex flex-col bg-gradient-to-br from-rose-100 via-pink-50 to-amber-50">
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <aside className="w-full md:w-1/3 lg:w-1/4 h-full flex flex-col border-r border-rose-200/60 bg-white/40">
                    <header className="p-4 border-b border-rose-200/80 flex justify-between items-center">
                        <h1 className="text-2xl font-bold font-serif text-rose-800">Your Circles</h1>
                        <Button onClick={() => setIsCircleModalOpen(true)} size="icon" className="bg-rose-600 hover:bg-rose-700 h-8 w-8">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-1">
                        ...
                        {circlesLoading && <p className="text-center text-rose-600 p-4">Loading...</p>}
                    </div>
                </aside>
            ...
            <AnimatePresence>
                {isUploadModalOpen && selectedCircle && (
                    <UploadMemoryModal onClose={() => setIsUploadModalOpen(false)} circleId={selectedCircle._id} />
                )}
                {isCircleModalOpen && (
                    <CreateCircleModal isOpen={isCircleModalOpen} onClose={() => setIsCircleModalOpen(false)} familyMembers={familyData?.members || []} />
                )}
                {selectedMemory && (
